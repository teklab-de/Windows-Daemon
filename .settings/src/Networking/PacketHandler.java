	...
	public void run() 
	{
		...
		if(data.startsWith("'useradd'"))
		{
			...
			if(split[1].contains("useradd"))
			{
				...
				{
					if(ftpserver.createFTPUsers(replaceusername, replacepassword, replacepfad) == "OK1")
					{
						try
						{
							handler.SendData("ID1");
		...
		if(data.startsWith("'games'"))
		{
			...
			if(split[3].startsWith("start"))
			{
				...
				if(split[13].startsWith(".bat"))
				{
					startbefehl = ""+split[13]+" "+split[14]+" "+split[15]+" "+split[16]+" "+split[17]+" "+split[18]+" "+split[19]+""+split[20]+"";	
					System.out.print("\nStartbefehl: " + startbefehl + "");
				}
				
				try
				{
					StartGameserver startgameserver = new StartGameserver(data, startbefehl, exename, pathgmo, path, user);
		...
		if(data.startsWith("'userdel'"))
		{
			...
			try
			{
				...
				{
					handler.SendData("ID2");
					writer.appendData("Server", "Das FTPConfigfile wurde nicht gelöscht, da es nicht existiert.");
				}
			}
			catch(Exception ex)
			{
